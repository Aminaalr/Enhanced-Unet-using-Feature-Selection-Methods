...
def loadFeaturedDataset():
    ...
    # Feature Selection
    totalFeature = 16 * 16
    # Create the RFECV and rank each pixel of 25% of Data
    selector = SelectKBest(f_classif, k=totalFeature)
    selector = RFECV(estimator=SVC(kernel="linear"), step=1, cv=StratifiedKFold(2), scoring="accuracy",
                     min_features_to_select=(totalFeature), n_jobs=-1)
