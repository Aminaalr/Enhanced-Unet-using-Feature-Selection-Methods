...
# Compile the model
unet_classification_Model.compile(optimizer=Adam(learning_rate=1e-3), loss='categorical_crossentropy', metrics=['accuracy'])
...
stTime = datetime.now()
# Train the model
history = unet_classification_Model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=no_epochs, batch_size=100, callbacks=callbacks)
#End Time
